...
| T-037 | Remplacer le lierre procédural, qui jure à côté des vrais modèles | T-036 ✅ |
...
| ID | Tâche | Résultat |
|---|---|---|
| T-039 | **Fidélité aux références : le laboratoire** | Le joueur a fourni des images de référence — labo blanc, baie vitrée courbe sur un désert en tempête, herbe dans les joints du carrelage. La direction était juste, la densité non. Trois changements structurels : la verrière passe de VOÛTE à **baie verticale courbe** (une voûte se lit comme une serre horticole, une baie courbe comme un poste d'observation), le plafond de verre devient **plein à caissons avec spots blancs froids** — la seconde température de lumière, sans laquelle toute la scène baignait dans le même ambre — et la végétation s'aligne sur les **joints du carrelage**, seule faiblesse d'un sol carrelé et signature immédiate du lieu abandonné. Le tablier de plancher sous la baie est ajouté dans `plan.js`, donc le semis le connaît : l'herbe pousse le long de la vitre sans qu'on l'y autorise à part. A résolu B-033 par effet de bord |
| T-036 | **Vraie végétation et semis sous contrainte** | Dix-neuf espèces CC0 (Quaternius), 0,8 Mo compressés. Mais l'essentiel n'est pas l'import : le joueur a signalé « de l'herbe dans le vide », et le décor était posé sans qu'on lui demande jamais s'il y avait du sol dessous — la question n'existait nulle part. La géométrie de chambre est sortie dans `plan.js`, lue par le bâtisseur ET par le semis : les mêmes dalles, donc plus de divergence possible. Le semis est une fonction PURE qui rend la classe de bogue impossible, pas seulement absente. 16 tests ; mutation, 9 sur 9 — dont deux tests réécrits parce qu'ils ne discriminaient rien, l'un recalculant de son côté la formule qu'il prétendait vérifier |
